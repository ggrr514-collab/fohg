...
cols=[('クラス',8),('出席番号',9),('氏名',14),
      ...
      ('AI評価\n（記述のみ暫定）',12),('AIコメント',34),
      ('★教師最終評価\n（記述＋発言）',13),
      ('自己評価',10),('評価差考察',26),('ふりかえり',26)]
for i,(name,w) in enumerate(cols,start=1):
    ws.column_dimensions[get_column_letter(i)].width=w
mg(ws,1,1,1,13,'教員用評価シート　「私」を消すもの、「私」を支えるもの（中3・全7時間／まとめ＝グループディスカッション）',
   F_TITLE,NAVY,center,h=28)
mg(ws,2,1,2,13,'目標：'+GOAL,F_GOAL,ORANGE_F,wrapc,h=32)
mg(ws,3,1,3,13,'★ 思考・判断・表現は【D〜G列の記述】と【当日の発言（「協議見取り」シート）】を合わせて'
                'J列で確定する。H列のAI評価は記述だけを見た暫定値。',F_RED,PINK,wrapc,h=28)
fills={8:PURPLE_F,9:PURPLE_F,10:PINK,11:GREEN_F,12:GREEN_F,13:GREEN_F}
...
for r in range(5,45):
    for i in range(1,14):
        ...
        c.alignment=center if i in (1,2,8,10,11) else wrap
        if i in fills: c.fill=fills[i]
    ws.row_dimensions[r].height=30
dv_ai.add('H5:H44'); dv_t.add('J5:J44')
ws.freeze_panes='D5'
...
for r,a,b in [(3,'AIモデル','gemini-2.0-flash'),(4,'Temperature',0.1),
              (6,'評価対象','論題1〜3の記述（D〜F列）＋見直し（G列）'),
              (7,'評価観点','思考・判断・表現のみ　5段階（S/A/B/C/D）'),
              (8,'知識・技能','この単元では評価しない（学期内の他単元・定期テストで見る）'),
              (10,'データ範囲','5行目〜44行目'),
...
mg(ws,17,1,17,2,'★ AIは条件②（当日の発言）を判定できない。H列は記述だけを見た暫定評価。'
                 '「協議見取り」シートのH列（発言の判定）と合わせて、J列で最終評価を確定する。',
   F_RED,None,wrap,h=44)
mg(ws,19,1,19,2,'★ 観点は9つから生徒が選ぶ（必修は「批評・評価」）。'
                 'どの観点を選んだかで評価を変えない。AIにも「別の観点をすすめない」と指示済み。',
   F_SMALL,None,wrap,h=40)
...
mg(ws,1,1,1,10,'協議見取り記録（2時・5〜7時）　―― 当日の発言を評価する表。'
                '思判表の条件②と、態度の評価材',F_TITLE,NAVY,center,h=28)
mg(ws,2,1,2,10,'★ 発話は残らない。ここを書かないと条件②（当日の発言）と態度の評価材が無くなる。'
                '5・6時は巡回しながらF〜J列を埋める。全発言を聞き取る必要はない。',
   F_RED,PINK,wrapc,h=32)
mg(ws,3,1,3,10,'★ E列は2時のおわりに記入し、【観点がばらけるように】班を組む。'
                '論題は3つ。1=仮面とデータは同じものか／2=訴えた人は救われたか／3=続けさせているのは誰か。'
                'いずれも二作にまたがる。発表する論題は当日に知らせる（生徒は3つ全部準備している）。',
   F_BODY,ORANGE_F,wrapc,h=32)
heads=['クラス','出席番号','氏名','班','選んだ観点（2時）','発表した\n論題',
       '当日の発言の要点','条件②\n○△×','見えなかったこと／述べ直した箇所','態度\nA/B/C']
...
mg(ws,46,1,46,10,'条件②の目安　○＝自分の観点を示し、本文の根拠を引いて主張した　'
                  '△＝発言したが根拠が本文に届かない　×＝発言していない',
   F_SMALL,GREEN_F,wrapc,h=22)
